...
class TestIgnoreComments:
    ...
    def test_sppd_ignore_marker_suppresses_matching_rule(self):
        code = "eval(user_input)  # sppd: ignore eval_exec_usage\n"
        result = Scanner().scan_source(code)
        assert result.findings == []

    def test_vibecodeguide_ignore_marker_suppresses_matching_rule(self):
        code = "eval(user_input)  # vibecodeguide: ignore eval_exec_usage\n"
        result = Scanner().scan_source(code)
        assert result.findings == []
